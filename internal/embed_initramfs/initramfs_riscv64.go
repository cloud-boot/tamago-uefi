...
// initramfsCPIOGz holds the riscv64 minimal initramfs (gzip-compressed
// cpio newc) — a single statically-linked riscv64 ELF /init that
// brings up the standard pseudo-filesystems, prints the Path D
// banner, and powers off via reboot(2). See init_src/init.go.
//
//go:embed initramfs_riscv64.cpio.gz
