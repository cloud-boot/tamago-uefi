#!/usr/bin/env bash
# M6.2 PR2 loong64 efipack smoke matrix.
#
# For each of {HTTP, HTTPS, OCI, EFIHANDOVER}:
#   1. Pack BOOTLOONGARCH64-<NAME>.EFI via go-coff/efipack (compress/flate).
#   2. Boot the packed PE under qemu-system-loongarch64 + EDK2 LoongArch
#      OVMF (virt machine, edk2-loongarch64-code.fd) with virtio-blk-pci
...
#
# Usage:
#     bash internal/efipacksmoke/run_loong64.sh
#
# Env overrides:
...
#     CLOUDBOOT_OVMF_LOONG64_CODE / _VARS  as in livehttp/run.sh
...
ROWS=(
    "HTTP|BOOTLOONGARCH64-HTTP.EFI|HTTP-GET OK"
    "HTTPS|BOOTLOONGARCH64-HTTPS.EFI|HTTPS-GET OK"
    "OCI|BOOTLOONGARCH64-OCI.EFI|OCI-FETCH OK"
    "EFIHANDOVER|BOOTLOONGARCH64-EFIHANDOVER.EFI|phase2-efi-handover: StartImage entering child"
)
...
WORK="$(mktemp -d -t efipack-smoke-loong64-XXXXXX)"
...
module efipacksmoke_pack_loong64
go 1.25
...
run_one() {
    ...
    mcopy -i "$esp" "$packed_path" "::/EFI/BOOT/BOOTLOONGARCH64.EFI"
    {
...
echo "=== M6.2 PR2 loong64 efipack smoke matrix ==="
