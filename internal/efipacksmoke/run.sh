#!/usr/bin/env bash
# M6.2 PR2 amd64 efipack smoke matrix.
#
# For each of {HTTP, HTTPS, OCI, EFIHANDOVER}:
#   1. Pack BOOTX64-<NAME>.EFI via go-coff/efipack (compress/flate).
#   2. Boot the packed PE under QEMU+EDK2 (q35, OVMF) with the same
#      device set the per-protocol live runner uses.
...
#
# Usage:
#     bash internal/efipacksmoke/run.sh
#
# Env overrides:
#     EFIPACK_TIMEOUT          per-row wall-clock cap in seconds (default 90)
#     EFIPACK_ROWS             space-separated row names (default: all four)
#     EFIPACK_DIR              repo path to go-coff/efipack
#                              (default: ../../../go-coff/efipack)
#     CLOUDBOOT_OVMF_AMD64_CODE / _VARS as in livehttp/run.sh
...
ROWS=(
    "HTTP|BOOTX64-HTTP.EFI|HTTP-GET OK"
    "HTTPS|BOOTX64-HTTPS.EFI|HTTPS-GET OK"
    "OCI|BOOTX64-OCI.EFI|OCI-FETCH OK"
    "EFIHANDOVER|BOOTX64-EFIHANDOVER.EFI|phase2-efi-handover: LoadImage OK"
)
...
WORK="$(mktemp -d -t efipack-smoke-XXXXXX)"
...
run_one() {
    ...
    local orig_sz packed_path packed_sz log esp work
    ...
    mcopy -i "$esp" "$packed_path" "::/EFI/BOOT/BOOTX64.EFI"
    {
    ...
    if [[ "$result" == "FAIL" || "$result" == "FAIL_GP" ]]; then
        echo "    log tail:"; tail -20 "$log" | sed 's/^/    | /'
    fi
}

echo "=== M6.2 PR2 amd64 efipack smoke matrix ==="
