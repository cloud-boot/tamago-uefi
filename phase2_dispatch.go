...
// `phase2_ministack_ping`, OR `phase2_dhcp4_acquire` (any
// combination) is set, this file owns the `runPhase2Probe` symbol
// that main.go calls. It invokes each enabled probe in sequence —
// Block-IO sink setup first (so subsequent prints are teed to disk),
// then PCI walk, then SNP walk, then the virtio-net TX/RX probe
// (M2), then the ministack ICMP-ping probe (M3-minimal), then the
// DHCPv4 acquire probe (M4), then a final Block-IO flush. Each
// probe's body lives in its own file and is gated on its own build
// tag; when a tag is NOT set, the matching `runXProbe` resolves to a
// no-op stub (phase2_*_stub.go).
//
...
//go:build (phase2_pcienum || phase2_snpenum || phase2_blkprintk || phase2_virtionet_tx || phase2_ministack_ping || phase2_dhcp4_acquire || phase2_http_get || phase2_https_get || phase2_oci_fetch) && !phase2_probe
