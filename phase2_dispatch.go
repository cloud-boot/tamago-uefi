...
// `phase2_virtionet_packed_tx` (any combination) is set, this file
...
// packed-ring virtio-net TX/RX probe (M2-A), then a final Block-IO
// flush. Each probe's body lives in its own file and is gated on
// its own build tag; when a tag is NOT set, the matching
...
//go:build (phase2_pcienum || phase2_snpenum || phase2_blkprintk || phase2_virtionet_tx || phase2_virtionet_packed_tx) && !phase2_probe

package main

// runPhase2Probe is the single entry point main.go calls when any
// Phase-2 probe is enabled. With every M1 + M1.5 + M1.6 + M2 + M2-A
// tag set it runs the Block-IO sink setup, then the PCI walk, then
// the SNP walk, then the split-ring virtio-net TX/RX probe, then the
// packed-ring virtio-net TX/RX probe (M2-A), then the Block-IO
// sentinel flush. With only a subset of the tags set, the missing
// probes resolve to no-op stubs and only the enabled ones run.
//
// M2-A (`phase2_virtionet_packed_tx`) is the R-M2c Option A
// experiment: rebuild the VZ TX path on the packed-virtqueue
// transport (Virtio 1.1 §2.7). Built as its own binary
// (BOOT*-VIRTIONETP.EFI). Mutually compatible with the split-ring M2
// probe at build time; the Taskfile builds them as separate binaries
// so a single hypervisor run exercises one transport at a time.
func runPhase2Probe() {
	runBlkPrintkSetup()
	runPCIEnumProbe()
	runSNPEnumProbe()
	runVirtioNetTxProbe()
	runVirtioNetPackedTxProbe()
