...
//   SP+40   ret status
//   SP+48   saved RA  (X1)
//   SP+56   saved S0  (X8)   — also Go's FP-equivalent under LP64
//   SP+64   saved S1  (X9)
//   SP+72   saved S2  (X18)
//   SP+80   saved S3  (X19)
//   SP+88   saved S4  (X20)
...
//   SP+144  padding (16-byte alignment)
// Total: 160 bytes.
...
TEXT ·loadFile_trampoline(SB),NOSPLIT|NOFRAME,$0
	...
	// Marshal EFI args (A0..A4) into Go ABI0 outgoing slots.
	MOV	A0, 0(X2)   // this
	MOV	A1, 8(X2)   // filePath
	MOV	A2, 16(X2)  // bp
	MOV	A3, 24(X2)  // sizeP
	MOV	A4, 32(X2)  // bufP
